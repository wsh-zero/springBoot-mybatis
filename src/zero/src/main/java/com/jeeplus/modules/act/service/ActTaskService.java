...
//	}
	...
	@Transactional(readOnly = false)
	public void approveTask(Act act) {
		// 设置意见
		act.setComment(("yes".equals(act.getFlag())?"[同意] ":"[驳回] ")+act.getComment());
		act.setFlag(("yes".equals(act.getFlag())?"approve":"noapprove"));
		act.preUpdate();
		// 对不同环节的业务逻辑进行操作
		String taskDefKey = act.getTaskDefKey();
		// 提交流程任务
		Map<String, Object> vars = Maps.newHashMap();
		vars.put("pass", "approve".equals(act.getFlag())? true : false);
	...
	@Transactional(readOnly = false)
	public void publishResume(Act act) {
		// 设置意见
		act.setComment(("yes".equals(act.getFlag())?"[完成] ":"[未发布] ")+act.getComment());
		act.setFlag(("yes".equals(act.getFlag())?"publish":"nopublish"));
		act.preUpdate();
		// 对不同环节的业务逻辑进行操作
		String taskDefKey = act.getTaskDefKey();
		// 提交流程任务
		Map<String, Object> vars = Maps.newHashMap();
		vars.put("pass", "publish".equals(act.getFlag())? true : false);
