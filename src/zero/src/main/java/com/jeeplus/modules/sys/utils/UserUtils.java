...
public class UserUtils {
	...
	private static StaffMapper staffMapper = SpringContextHolder.getBean(StaffMapper.class);
...
//		getCacheMap().remove(key);
	...
	/**
	 * 导出Excel使用，根据名字转换为id(员工信息)
	 */
	public static Staff getByStaffName(String name){
		Staff s = new Staff();
		List<Staff> list = staffMapper.findList(s);
		if (list.size()>0){
			return list.get(0);
		}else {
			return new Staff();
	...
	/**
	 * 导出Excel使用，根据名字转换为id(机构管理)
	 */
	public static Office getByOrgName(String name){
		Office o = new Office();
		List<Office> list = officeMapper.findList(o);
		if (list.size()>0){
			return list.get(0);
